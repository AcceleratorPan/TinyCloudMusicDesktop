...
swiftc -j 1 -disable-batch-mode -parse-as-library -warnings-as-errors \
  Sources/TinyCloudMusic/Models.swift \
  Sources/TinyCloudMusic/Repository.swift \
  Sources/TinyCloudMusic/TrackCache.swift \
...
swiftc -j 1 -disable-batch-mode -warnings-as-errors \
  Sources/TinyCloudMusic/EAPITransport.swift \
  Sources/TinyCloudMusic/CredentialSnapshot.swift \
  Sources/TinyCloudMusic/CredentialStore.swift \
...
swiftc -j 1 -disable-batch-mode -D TRACK_CACHE_CHECK -warnings-as-errors \
  Sources/TinyCloudMusic/TrackCache.swift \
...
swiftc -j 1 -disable-batch-mode -parse-as-library -warnings-as-errors Checks/APIParityCheck.swift -o /tmp/tinycloudmusic-api-parity-check
/tmp/tinycloudmusic-api-parity-check
swiftc -j 1 -disable-batch-mode -warnings-as-errors "${COMMON_SOURCES[@]}" Checks/WriteAPIContractCheck.swift -o /tmp/tinycloudmusic-write-api-check
/tmp/tinycloudmusic-write-api-check
swiftc -j 1 -disable-batch-mode -warnings-as-errors "${COMMON_SOURCES[@]}" Checks/PlaybackAvailabilityCheck.swift -o /tmp/tinycloudmusic-playback-availability-check
/tmp/tinycloudmusic-playback-availability-check
swiftc -j 1 -disable-batch-mode -warnings-as-errors "${COMMON_SOURCES[@]}" Checks/RecommendationMemoryCheck.swift -o /tmp/tinycloudmusic-recommendation-memory-check
/tmp/tinycloudmusic-recommendation-memory-check
swiftc -j 1 -disable-batch-mode -D LISTENING_REPORT_CHECK -warnings-as-errors \
  "${COMMON_SOURCES[@]}" \
...
swiftc -j 1 -disable-batch-mode -D VIDEO_CHECK -warnings-as-errors \
  "${COMMON_SOURCES[@]}" \
...
swiftc -j 1 -disable-batch-mode -D AUDIO_CONTENT_CHECK -warnings-as-errors \
  "${COMMON_SOURCES[@]}" \
...
swiftc -j 1 -disable-batch-mode -D AUDIO_UPLOAD_CHECK -warnings-as-errors \
  "${COMMON_SOURCES[@]}" \
...
swiftc -j 1 -disable-batch-mode -D MUSIC_KNOWLEDGE_CHECK -warnings-as-errors \
  Sources/TinyCloudMusic/Models.swift \
  Sources/TinyCloudMusic/CredentialSnapshot.swift \
  Sources/TinyCloudMusic/CredentialStore.swift \
  Sources/TinyCloudMusic/EAPITransport.swift \
...
swiftc -j 1 -disable-batch-mode -D CLOUD_MUSIC_CHECK -warnings-as-errors \
  "${COMMON_SOURCES[@]}" \
  Sources/TinyCloudMusic/MusicDownload.swift \
...
swiftc -j 1 -disable-batch-mode -D MUSIC_DOWNLOAD_CHECK -warnings-as-errors \
  "${COMMON_SOURCES[@]}" \
  Sources/TinyCloudMusic/MusicDownload.swift \
  Tests/TinyCloudMusicTests/MusicDownloadTests.swift \
  -o /tmp/tinycloudmusic-download-check
/tmp/tinycloudmusic-download-check
swiftc -j 1 -disable-batch-mode -warnings-as-errors "${COMMON_SOURCES[@]}" Checks/LiveAPICheck.swift -o /tmp/tinycloudmusic-live-api-check
