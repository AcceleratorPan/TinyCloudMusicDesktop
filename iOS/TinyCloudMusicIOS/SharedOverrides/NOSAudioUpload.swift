...
struct NOSAudioUpload: Sendable {
    ...
    func completePodcastMultipart(
        ...
        let body = NOSMultipartXML.body(parts: parts)
        var components = URLComponents(url: try podcastObjectURL(allocation.objectKey), resolvingAgainstBaseURL: false)!
...
enum NOSMultipartXML {
    static func body(parts: [AudioUploadPart]) -> Data {
        var resume = PodcastUploadResume()
        resume.parts = parts
        let elements = resume.stableParts.map { part in
            "<Part><PartNumber>\(part.number)</PartNumber><ETag>\(escape(part.etag))</ETag></Part>"
        }
        return Data(("<CompleteMultipartUpload>" + elements.joined()
