...
final class ListenTogetherController {
    ...
    var requiresShutdown: Bool { room != nil || roomOperationTask != nil }

    func updateAccount(_ userID: Int64?) {
        accountRevision = accountRevision == .max ? 1 : accountRevision + 1
        let revision = accountRevision
        accountTask?.cancel()
        accountTask = Task { @MainActor [weak self] in
            await self?.updateAccountState(userID, revision: revision)
        }
    }

    private func updateAccountState(_ userID: Int64?, revision: Int) async {
        roomOperationsBlocked = true
        if let roomOperationTask { await roomOperationTask.value }
        guard revision == accountRevision, !Task.isCancelled else { return }
        defer {
            if revision == accountRevision { roomOperationsBlocked = userID == nil }
        }
        if currentUserID == userID, room != nil { return }
        let previousRoom = room
        invalidateSessionWork()
        player.controlInterceptor = nil
        player.setControlInteractionLocked(false)
        room = nil
        if userID == nil, let previousRoom {
            _ = try? await service.endRoomConfirmed(roomID: previousRoom.id)
            guard revision == accountRevision, !Task.isCancelled else { return }
        }
        ...
        do {
            let status = try await service.status(currentUserID: userID)
    ...
    private func performCreateRoom() async {
        guard let userID = authenticatedUserID(), room == nil else { return }
        let sessionGeneration = beginRoomOperation(phase: .creating)
        do {
            let createdRoom = try await service.createRoom(currentUserID: userID)
            try Task.checkCancellation()
            guard isCurrent(sessionGeneration, userID: userID) else { return }
            try await establish(createdRoom, generation: sessionGeneration, bootstrapHost: true)
        } catch is CancellationError {
        } catch {
    ...
    private func performCheckInvitation(_ invitation: ListenTogetherInvitation) async {
        guard authenticatedUserID() != nil, room == nil else { return }
        let sessionGeneration = beginRoomOperation(phase: .checking(invitation))
        do {
            let check = try await service.checkInvitation(invitation)
            try Task.checkCancellation()
    ...
    private func performJoin(_ invitation: ListenTogetherInvitation) async {
        ...
        do {
            let joinedRoom = try await service.acceptInvitation(invitation, currentUserID: userID)
            try Task.checkCancellation()
            guard isCurrent(sessionGeneration, userID: userID) else { return }
            try await establish(joinedRoom, generation: sessionGeneration, bootstrapHost: false)
        } catch is CancellationError {
        } catch {
            fail(error, generation: sessionGeneration)
        }
    ...
    func sleep() async {
        ...
        if let roomOperationTask { await roomOperationTask.value }
        guard isSleeping, let currentRoom = room else {
            isSleeping = false
            return
        }
    ...
    func prepareForLogout() async {
        ...
        accountTask = nil
        if let roomOperationTask { await roomOperationTask.value }
        guard currentUserID != nil || room != nil else { return }
    ...
    private func performLogoutCleanup() async {
        ...
        invalidateSessionWork()
        if let currentRoom {
            _ = try? await service.endRoomConfirmed(roomID: currentRoom.id)
        }
    ...
    private func establish(
        ...
        do {
            ...
            try Task.checkCancellation()
            guard !isSleeping,
                  isCurrent(sessionGeneration, roomID: newRoom.id)
            ...
            if bootstrapHost {
                try await reportHostSnapshot(roomID: newRoom.id, userID: newRoom.creatorID)
            } else {
                try await synchronizeState(roomID: newRoom.id, generation: sessionGeneration)
            }
    ...
    private func reportHostSnapshot(roomID: String, userID: Int64) async throws {
    ...
    private func reportHostProgress(roomID: String, userID: Int64) async throws {
    ...
    private func sendHostSnapshot(includePlaylist: Bool) {
        ...
        hostSnapshotTask = Task { @MainActor [weak self] in
            ...
            do {
                try Task.checkCancellation()
                if includePlaylist {
                    try await self.reportHostSnapshot(roomID: currentRoom.id, userID: userID)
                } else {
                    try await self.reportHostProgress(roomID: currentRoom.id, userID: userID)
                }
                try Task.checkCancellation()
    ...
    private func intercept(
        ...
        guard !isSendingCommand, let userID = currentUserID else {
        ...
        commandTask = Task { @MainActor [weak self] in
            ...
            do {
                try await self.report(intent, roomID: currentRoom.id, userID: userID)
                try Task.checkCancellation()
                guard self.isCurrent(sessionGeneration, roomID: currentRoom.id) else { return }
                commit()
            } catch is CancellationError {
    ...
    private func report(_ intent: PlayerControlIntent, roomID: String, userID: Int64) async throws {
        if let order = intent.queue {
            ...
            guard try await service.reportPlaylistCommandConfirmed(roomID: roomID, command: command) else {
                throw ListenTogetherControllerError.commandRejected
            }
        }
        if let intent = intent.play {
            let command = try playCommand(from: intent)
            guard try await service.reportPlayCommandConfirmed(roomID: roomID, command: command) else {
                throw ListenTogetherControllerError.commandRejected
            }
        }
    }
    ...
    private func runReconciliation(generation sessionGeneration: Int) async {
        ...
        while reconciliationRequested, generation == sessionGeneration, !Task.isCancelled {
            ...
            reconciliationRequiresReconnect = false
            if reconnect {
                guard await reconnectWithRetries(generation: sessionGeneration) else { return }
                continue
            }
            do {
                let previousError = errorMessage
                try await reconcile(generation: sessionGeneration, reconnectRealtime: false)
    ...
    private func reconnectWithRetries(generation sessionGeneration: Int) async -> Bool {
        for attempt in 1...Self.maximumReconnectAttempts {
            ...
            do {
                errorMessage = nil
                try await reconcile(generation: sessionGeneration, reconnectRealtime: true)
    ...
    private func reconcile(generation sessionGeneration: Int, reconnectRealtime: Bool) async throws {
        ...
        try await synchronizeState(roomID: statusRoom.id, generation: sessionGeneration)
        applyPendingRemoteEvent()
    }

    private func synchronizeState(roomID: String, generation sessionGeneration: Int) async throws {
    ...
    private func exitRoom() async {
        guard let currentRoom = room, let userID = currentUserID else { return }
        let sessionGeneration = invalidateSessionWork(retainingRoomState: true)
        room = currentRoom
        phase = .ending(currentRoom)
        player.setControlInteractionLocked(true)
        do {
            guard try await service.endRoomConfirmed(roomID: currentRoom.id) else {
            ...
            guard isCurrent(sessionGeneration, userID: userID, roomID: currentRoom.id) else { return }
            if let status = try? await service.status(currentUserID: userID), !status.inRoom {
                clearSession(phase: .ended(reason: nil))
                await realtime.disconnect()
                return
            }
    ...
    private func finishSession(reason: String?) {
        ...
        Task { @MainActor [weak self] in await self?.realtime.disconnect() }
    }
    ...
    private func performRoomOperation(
        ...
        guard allowWhenBlocked || !roomOperationsBlocked else { return }
